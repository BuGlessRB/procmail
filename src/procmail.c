...
privileged:
  ...
  thebody=themail=malloc((tobesent=STRLEN(From_)+strlen(buf2))+STRLEN(From_));
  filled=0;
...
Setuser:
  ...
  */
  if(!stat(buf,&stbuf)&&
   (((gid_t)(sgid=stbuf.st_gid)==getegid()||(rc=rc_NOSGID,0))&&
  ...
  */
  ...
  if(!Privileged&&!(stbuf.st_mode&S_IWGRP))   /* lockfile unrightfully owned */
   { succeed=1;
boglock:
     rename(buf2,buf);	 /* try and rename the bogus lockfile out of the way */
   }
  else
